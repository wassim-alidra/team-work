...
import { Users, Package, ShoppingBag, BarChart3, ChevronDown } from 'lucide-react';
...
const SetisticsDashboard = () => {
    ...
    const fetchAllStats = async () => {
        setLoading(true);
        try {
            const [farmersRes, salesRes, productsRes] = await Promise.all([
    ...
    return (
        <div className="space-y-lg p-lg animate-in pb-xl">
            ...
            <section className="bg-surface-container-lowest rounded-xl p-lg shadow-[0_4px_20px_rgba(26,58,52,0.05)] border border-outline-variant/30">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-md mb-lg">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-h3 text-h3 text-on-surface">Weekly Sales Analysis</h3>
                            <p className="font-body-sm text-body-sm text-on-surface-variant">Sales quantity trends over time for specific products.</p>
                        </div>
                    </div>
                    ...
                    <div className="relative min-w-[200px]">
                        <select 
                            ...
                            className="w-full pl-4 pr-10 py-2 bg-surface border border-outline-variant rounded-lg font-body-sm text-body-sm text-on-surface appearance-none focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all cursor-pointer"
                        >
                    ...
                    {weeklySalesData.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                                ...
                                <XAxis 
                                    dataKey="week" 
                                    tick={{ fill: '#414846', fontSize: 12, fontFamily: 'Inter' }}
                                    tickFormatter={(val) => `Week of ${new Date(val).toLocaleDateString(undefined, {month: 'short', day: 'numeric'})}`}
                                />
                                <YAxis tick={{ fill: '#414846', fontSize: 12, fontFamily: 'Inter' }} />
                                <Tooltip 
                                ...
                                />
                                <Bar dataKey="quantity" fill="#2D6A4F" radius={[4, 4, 0, 0]} barSize={40} />
                            </BarChart>
                        </ResponsiveContainer>
                    ) : (
                        <div className="flex flex-col items-center justify-center h-full text-on-surface-variant italic border-2 border-dashed border-outline-variant/30 rounded-xl">
                            ...
                            <p>{selectedProduct ? "No sales recorded for this product yet." : "Select a product to view its sales history."}</p>
                        </div>
                    )}
                </div>
            </section>
